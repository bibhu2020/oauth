...
class CacheMiddleware {
  ...
  checkCache = async (req, res, next) => {
    try {
      ...
      if (cachedContent) {
        return res.send(cachedContent); // Use return to ensure no further code is executed
      } else {
        ...
        res.send = async (body) => {
          ...
          return originalSend(body); // Return original send to ensure response is sent
        };
